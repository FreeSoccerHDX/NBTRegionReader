...
public class Section implements Comparable<Section> {
	...
	private static final CompoundTag AIR_COMPOUND = new CompoundTag();
	static{
		AIR_COMPOUND.put("Name",new StringTag("minecraft:air"));
	}

	private CompoundTag data;
	private Map<String, List<PaletteIndex>> valueIndexedPalette = new HashMap<>();
	private Map<String, List<PaletteIndex>> valueIndexedBiomePalette = new HashMap<>();
	private ListTag<CompoundTag> palette;
	private ListTag<StringTag> biomePalette;
	...
	//	for(String key : sectionRoot.keySet()){
	//		System.out.println("Keys: " + key);
	//	}

		CompoundTag blockStateTag = sectionRoot.getCompoundTag("block_states");
		CompoundTag biomesTag = sectionRoot.getCompoundTag("biomes");

		if(blockStateTag != null){
			ListTag<?> rawPalette = blockStateTag.getListTag("palette");
			if (rawPalette != null) {
				...
				LongArrayTag blockStates = blockStateTag.getLongArrayTag("data");
		...
		if(biomesTag != null){
			ListTag<?> rawPalette = biomesTag.getListTag("palette");
			if (rawPalette != null) {
				biomePalette = rawPalette.asStringTagList();

				for (int i = 0; i < biomePalette.size(); i++) {
					StringTag data = biomePalette.get(i);
					CompoundTag compoundTag = new CompoundTag();
					compoundTag.putString("Name",data.getValue());
					putValueIndexedBiomePalette(compoundTag, i);
				}
				LongArrayTag biomes = biomesTag.getLongArrayTag("data");

				this.blockBiomes = biomes != null ? biomes.getValue() : null;
			}
		}
	...
	void putValueIndexedBiomePalette(CompoundTag data, int index) {
		PaletteIndex leaf = new PaletteIndex(data, index);
		String name = data.getString("Name");
		List<PaletteIndex> leaves = valueIndexedBiomePalette.get(name);
		if (leaves == null) {
			leaves = new ArrayList<>(1);
			leaves.add(leaf);
			valueIndexedBiomePalette.put(name, leaves);
		} else {
	...
	* */
	...
	public CompoundTag getBlockBiomeAt(int blockX, int blockY, int blockZ) {
		return getBlockBiomeAt(getBiomeIndex(blockX&3, blockY&3, blockZ&3));
	}

	private CompoundTag getBlockBiomeAt(int index) {

		if(this.blockBiomes == null){
			if(this.biomePalette != null){
				if(this.biomePalette.size() == 1){
					StringTag biomeTag = biomePalette.get(0);
					CompoundTag compoundTag = new CompoundTag();
					compoundTag.putString("Name",biomeTag.getValue());
					return compoundTag;
				}
			}
			return null;
		...
		int paletteIndex = getBiomePaletteIndex(index);
	//	System.out.println("biomeindex: " + index + " paletteIndex=" + paletteIndex);
		if(biomePalette.size() <= paletteIndex || paletteIndex < 0){
			System.out.println("ERROR-BIOME: index="+paletteIndex+" and size="+biomePalette.size() + " paletteIndex="+index);
			try {
				Thread.sleep(10);
			}catch (Exception ex){
				ex.printStackTrace();
		...
		StringTag biomeTag = biomePalette.get(paletteIndex);
		CompoundTag compoundTag = new CompoundTag();
		compoundTag.putString("Name",biomeTag.getValue());
		return compoundTag;
	}
	...
	int getBiomeIndex(int biomeX, int biomeY, int biomeZ) {
		...
		//return (biomeY << sizeBits | biomeZ) << sizeBits | biomeX;

		//return biomeY * 16 + biomeZ * 4 + biomeX;
	}
	public int getBiomePaletteIndex(int blockStateIndex) {
		int bits = blockBiomes.length;

		int indicesPerLong = (int) (64D / bits);
		int blockStatesIndex = blockStateIndex / indicesPerLong;
		int startBit = (blockStateIndex % indicesPerLong) * bits;
		return (int) bitRange(blockBiomes[blockStatesIndex], startBit, startBit + bits);

	}

	private CompoundTag getBlockStateAt(int index) {
		if(this.blockStates == null){
			return AIR_COMPOUND.clone();
		}
		int paletteIndex = getPaletteIndex(index);
		if(palette.size() <= paletteIndex){
			System.out.println("ERROR-STATE: index="+paletteIndex+" and size="+palette.size());
			try {
				Thread.sleep(100);
			}catch (Exception ex){
				ex.printStackTrace();
	...
	int getBlockIndex(int blockX, int blockY, int blockZ) {
		...
		//int sizeBits = 4;
		//return (blockY << sizeBits | blockZ) << sizeBits | blockX;
	}
