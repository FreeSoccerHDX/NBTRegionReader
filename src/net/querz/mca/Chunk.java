...
public class Chunk implements Iterable<Section> {
	...
	public int getMaxY(){
		return sections.size()*16+getMinY();
	}
	...
	private void initReferences(long loadFlags) {
		...
		for(String key : data.keySet()){
			System.out.println("Data: -> "+key + " = "+data.get(key).getID());
		}
		*/
		...
		this.posX = data.getInt("xPos");
		this.posY = data.getInt("yPos");
		this.posZ = data.getInt("zPos");
		this.lastUpdate = data.getLong("LastUpdate");
		this.dataVersion = data.getInt("DataVersion");

		ListTag<CompoundTag> sectionsTag = data.getListTag("sections").asCompoundTagList();
...
/*
	...
	*/
	...
	public BiomeData getBiomeData(int blockX, int blockY, int blockZ){
		CompoundTag tag = getBiomeCompoundAt(blockX,blockY,blockZ);
		if(tag == null){
			return null;
		}

		return BiomeData.getBiomeDataByName(tag.getString("Name"));
	}

	public CompoundTag getBiomeCompoundAt(int blockX, int blockY, int blockZ) {
